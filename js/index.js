...
async function loadLocation() {
  ...
  console.log(
    ...
    station_id == undefined
      ? `http://climate.rotriw.com/point/climate?lat=${parseFloat(
          loc.lat
        )}&lon=${parseFloat(loc.lon)}`
      : `http://climate.rotriw.com/station/climate?id=${station_id}`
  );
  $.ajax({
    url:
      station_id == undefined
...
async function loadView() {
  ...
  .search({
    minCharacters : 3,
    apiSettings: {
      url: 'http://climate.rotriw.com/station/find?name={query}',
